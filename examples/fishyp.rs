...
use pix::{Ch8, Format, RasterBuilder, Rgba8, PremulRgba8};
...
fn main() -> Result<(), std::io::Error> {
    ...
    let mut r = RasterBuilder::<PremulRgba8>::new().with_pixels(p.width(), p.height(), v);
    raster_over(
        &mut r,
    ...
    p.clear_mask();
    raster_over(&mut r, p.stroke(&fish), PremulRgba8::new(255, 208, 208), 0, 0);
    p.clear_mask();
